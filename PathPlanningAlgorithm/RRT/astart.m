...
cntTime = cntTime + toc;
cntPath = cntPath + pathLength;

fprintf('processing time=%d \nPath Length=%d \n\n', toc,pathLength); 
imshow(map);rectangle('position',[1 1 size(map)-1],'edgecolor','k');
line(path(:,2),path(:,1));

alpha = 0.5;    % 与原始曲线的贴合程度    
beta = 0.5;     % 平滑后的曲线平滑程度

pathSmooth = path;

for k = 1:10
    for i = 2:1:(length(path) - 1) % 不优化起点与终点
        pathSmooth(i,1) = pathSmooth(i,1) + alpha*(path(i,1) - pathSmooth(i,1)) + beta*(pathSmooth(i-1,1) - 2*pathSmooth(i,1) + pathSmooth(i+1,1));
        pathSmooth(i,2) = pathSmooth(i,2) + alpha*(path(i,2) - pathSmooth(i,2)) + beta*(pathSmooth(i-1,2) - 2*pathSmooth(i,2) + pathSmooth(i+1,2));
    end
end
line(pathSmooth(:,2),pathSmooth(:,1),'color','r');
end
fprintf('Avg processing time=%d \n Avg Path Length=%d \n\n', cntTime/10, cntPath/10);
